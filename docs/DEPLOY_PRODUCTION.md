...
```bash
git clone git@github.com:theirstory/ts-portal-ishs.git
cd ts-portal-ishs
```
...
```bash
./scripts/deploy/export-weaviate-data.sh "$PWD/weaviate-data.tar.gz" ts-portal-ishs_weaviate_data root@YOUR_DROPLET_IP /root/ts-portal-ishs
```
